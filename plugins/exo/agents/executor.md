...
- **Usa la memoria si aplica (degradable).** Si tu brief referencia notas de memoria (permalinks / memory packet), léelas antes de empezar con `exo search --db ~/.exo/index.db --type hybrid --json "<query>"` (devuelve `permalink`, `score`, `ruta`) o `exo targets <topic> --json`. Si el engine no responde, sigue sin bloquearte.
