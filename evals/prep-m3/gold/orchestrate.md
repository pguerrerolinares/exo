...
- [ ] Degradación con aviso visible al armar el memory packet: probe del CLI/KB → si no hay engine/KB, brief sin packet + aviso visible; NUNCA bloquear el dispatch por esto — spec prep-M3 §3.4 ("Skills con dependencia de instancia en v1: `documenta` … y `orchestrate` (memory packet)") + framework §5.1 (patrón único de degradación). [Ítem espejo del de documenta.md línea 10]
