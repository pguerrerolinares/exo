...
- **Objeto**: rama `m0-fase0` (11 commits, `bc34ed5..dc74d26`) para merge a `master`. El verdict experimental (f80393a) está fuera de alcance por mandato — este gate adjudica solo la calidad de la rama.

## GATE: APROBADA

Fast-forward puro (merge-base de master y m0-fase0 == master == `bc34ed5`; sin conflictos posibles). Los tres Minors del ledger van como deuda documentada, ninguno bloquea (triage abajo).
...
3. **Sin residuo en el árbol**: `git ls-tree -r dc74d26` no contiene temporales, logs, `.tmp`, sidecar ni pycache. Fuera de `evals/` y `docs/` solo cambia `.gitignore` (+5 líneas, correctas). El sidecar `.min_similarity_backup.json` no existe en disco (restauración de replay.py funcionó). Único residuo: `harness/__pycache__/` **untracked** en el working tree — no entra al merge; housekeeping opcional añadir `__pycache__/` al .gitignore.
4. **Audit trail íntegro y trazable**: orden de commits correcto — baseline medido (6d74513) → gate sellado (ec3d512) → brazos (5c102fc) → fix de medición (58e56f3) → verdict (f80393a) → aplicación (dc74d26). `git log --follow -- gate.md` muestra **un solo commit** (ec3d512): el gate no se tocó después de sellado. Los números del verdict (43/55, 41/55, 7/0, 7/2, 26/55, 18/55) cuadran con `metrics-*.md`, que cuadran con los jsonl.
